...
class AddHorseStep2Fragment : BaseBindingFragment<FragmentAddHorseStep2Binding>() {
    ...
    private fun setUpListeners() {
        ...
        binding?.btnAddProduct?.setOnClickListener {
            if (isDataValid()) {
                viewModel.addHorse(viewModel.getHorseRequest(
    ...
    private fun setUpCategories() {
        ...
        binding?.spinnerSaleType?.setOnSpinnerItemSelectedListener<String> { oldIndex, oldItem, newIndex, newItem ->
            ...
            if (newIndex == 0) {
                ...
                binding?.tvPriceTitle?.text = resources.getString(R.string.enter_the_starting_price_of_the_auction)
            }
        }
        saleTypeSpinnerAdapter.setItems(arrayListOf(
